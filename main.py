...
if __name__ == "__main__":
    ...
    train_loader = get_loader(args.batch_size, args.train_img_dir, args.train_attr_path, args.selected_attrs,
                              args.celeba_crop_size,
                              args.input_size[1], dataset='CelebA', mode='train', num_workers=args.num_workers)

    test_loader = get_loader(args.batch_size, args.test_img_dir, args.test_attr_path, args.selected_attrs,
                             args.celeba_crop_size, args.input_size[1], dataset='CelebA', mode='test',
    ...
    for i in range(n_epochs + 1):
        ...
        for j, (x, label) in enumerate(train_loader):
            ...
            # encoder
            loss_encoder.backward(retain_graph=True)  # someone likes to clamp the grad here:
            [p.grad.data.clamp_(-1,1) for p in net.encoder.parameters()]
            net.zero_grad()  # clean others, so they are not afflicted by encoder loss
        ...
        for j, (x, label) in enumerate(test_loader):
            ...
            out = x.data.cpu()
            out = (out + 1) / 2
            save_image(vutils.make_grid(out[:64], padding=5, normalize=True).cpu(), './result/original%s.png' % i,
                        nrow=8)
            ...
            save_image(vutils.make_grid(out[:64], padding=5, normalize=True).cpu(),
                        './result/reconstructed%s.png' % i, nrow=8)

            out = net(None, 100)  ##out=x_p
            out = out.data.cpu()
            out = (out + 1) / 2
            save_image(vutils.make_grid(out[:64], padding=5, normalize=True).cpu(), './result/generated%s.png' % i,
                        nrow=8)
